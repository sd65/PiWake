...
$lines = file('/usr/share/nginx/www/piwake/vcs/SRC/VCSALL');
	...
	foreach ($lines as $lineNumber => $lineContent) // Pour chaque ligne
    {
		...
		if(stripos($lineContent, "BEGIN:VEVENT") !== false)
		{
			...
			$matiere = preg_replace("#=C3=A8#", "è",$matiere) ; // & corrige les e
			...
			echo "Debug : " . $semestre ." # " . $semaine ." # " .$nomJour." # " .$annee." # " .$mois." # " .$jour." # " . $matiere." # " . $prof." # " . $salle. $type. " # " .$startHeure. " # " .$endHeure . "</br>" ;
...
// On flush tout pour éviter les doublons			
$bdd->exec("DELETE FROM EDT_MMI WHERE 1") ; 


$lines = file('/usr/share/nginx/www/piwake/vcs/MMI/VCSALL');
	...
	foreach ($lines as $lineNumber => $lineContent) // Pour chaque ligne
    {
		...
		if(stripos($lineContent, "BEGIN:VEVENT") !== false)
		{
			...
			$req  = $bdd->prepare("INSERT INTO EDT_MMI (semestre, semaine, nomJour, annee, mois, jour, matiere, prof, salle, type, startHeure, endHeure) VALUES (:semestre, :semaine, :nomJour, :annee, :mois, :jour, :matiere, :prof, :salle, :type, :startHeure, :endHeure)");
